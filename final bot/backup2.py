...
cursor = db.cursor()
url_im = cursor.execute("SELECT link FROM users WHERE userid = ?", [833721535]).fetchone()[0] # Тут лежит ссылка на фон
resp = requests.get(url_im, stream=True).raw
heh = Image.open(resp) #Вставка фона

fg = Image.open('C:/Users/kvach/Desktop/tgbote/newfon21.png').resize((1280, 800), Image.ANTIALIAS) #Вставка шаблона
...
def main():
	def meme():
		...
		db = sqlite3.connect('users_settings.db')
		cursor = db.cursor()
		place = cursor.execute("SELECT place FROM users WHERE userid = ?", [833721535]).fetchone()[0]
		...
		pogoda = a
		font = ImageFont.truetype('C:/Users/kvach/Desktop/tgbote/Comic_CAT.otf', size=28)
	...
	def meme_1():
		...
		font = ImageFont.truetype('C:/Users/kvach/Desktop/tgbote/Comic_CAT.otf', size=36)
		draw_text = ImageDraw.Draw(heh)
	...
	#def meme_4():
		#db = sqlite3.connect('users_settings.db')
		#cursor = db.cursor()
		#zam = cursor.execute("SELECT zametka FROM users WHERE userid = ?", [833721535]).fetchone()[0]
		#zametka = zam[0:30] + "\n\n"
		#zametka += zam[30:60] + "\n\n"
		#zametka += zam[60:90] + "\n\n"
		#zametka += zam[90:120] + "\n\n"
	...
	def meme_5():
		...
		for y in articles:
			...
			font = ImageFont.truetype('C:/Users/kvach/Desktop/tgbote/Comic_CAT.otf', size=15)
			draw_text = ImageDraw.Draw(heh)
	...
	heh.save('C:/Users/kvach/Desktop/tgbote/newfon.png')
